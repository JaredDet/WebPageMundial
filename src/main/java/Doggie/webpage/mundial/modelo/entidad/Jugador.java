...
public class Jugador {
    ...
    @JsonManagedReference
    @OneToMany(mappedBy = "jugador", fetch = FetchType.EAGER, cascade = CascadeType.ALL)
    ...
    public String getNombreTitular() {
        ...
        return nombres.length > 1 ? nombres[0].charAt(0) + ". " + nombres[nombres.length - 1]
                : nombre;
    }

    public boolean anotoGolesEnPartido(Partido partido) {
        var golesPorPartido = getGolesPorPartido(partido);
        return !golesPorPartido.isEmpty();
    }
    ...
    public boolean tuvoTarjetaAmarillaEnPartido(Partido partido) {
        var tarjetasPorPartido = getTarjetasPorPartido(partido);
        return tarjetasPorPartido.stream().anyMatch(tarjeta -> tarjeta.getColor().equals(Color.AMARILLA));
    }

    public boolean tuvoTarjetaRojaEnPartido(Partido partido) {
        var tarjetasPorPartido = getTarjetasPorPartido(partido);
        return tarjetasPorPartido.stream().anyMatch(tarjeta -> tarjeta.getColor().equals(Color.ROJA));
    }
