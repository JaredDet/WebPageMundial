...
public interface RepositorioPartido extends JpaRepository<Partido, Long> {
    ...
    @Query("SELECT p FROM Partido p JOIN FETCH p.convocados c JOIN FETCH c.jugador WHERE p.partidoId = :partidoId")
    Partido findPartidoByIDForPartido(@Param("partidoId") Long partidoId);
    ...
    @Query(value = "SELECT DISTINCT partidos.* FROM Grupos " +
            ...
            "ON Participantes.Estadisticas_estadistica_id = Estadisticas.estadistica_id " +
